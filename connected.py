...
def detect_red_led(frame: np.ndarray):
    ...
    connectedComponentsWithStats 사용 (findContours 대비 경량).
    dilation 제거: MIN_AREA=1로 1px도 검출 가능하므로 불필요.
    """
    ...
    # stats 컬럼: [LEFT, TOP, WIDTH, HEIGHT, AREA]
    results = []
    for label in range(1, num_labels):          # 0 = 배경 스킵
        area = stats[label, cv2.CC_STAT_AREA]
        if not (MIN_AREA <= area <= MAX_AREA):
            continue

        cx, cy = centroids[label]               # float64, 이미 무게중심
