...
class LEDTrackerCA:
    ...
    def update(self, yaw_world: float, pitch_world: float):
        ...
        return (
            byaw, bpitch,
            corrected[2, 0],   # ω_yaw   — correction 기준 각속도
            corrected[3, 0],   # ω_pitch — correction 기준 각속도
            corrected[4, 0],   # α_yaw
    ...
    def predict_only(self):
        ...
        print(f"[PREDICT_ONLY] miss={self.miss_count}  ω_yaw={wy:.3f}  ω_pitch={wp:.3f}")
...
def draw_results(frame: np.ndarray, prediction: tuple):
    ...
    if prediction is not None:
        # prediction은 월드 각도이므로 화면 표시용으로 다시 픽셀 변환 불필요.
        # 대신 화면 중심에 조준 마커만 표시 (서보가 맞추고 있다는 의미)
        cv2.drawMarker(vis, (xy2angle.getcx(), xy2angle.getcy()), (255, 255, 255),
...
def main():
    ...
    tracker = LEDTrackerCA(
        dt=1/30,
        pos_noise=1e-2,   # 각도 단위 [deg²]
        vel_noise=5e-1,   # 각속도 단위 [deg²/s²]
        acc_noise=1e-2,
        meas_noise=0.3,
        max_missing=5,
