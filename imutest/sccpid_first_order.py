...
class ServoController:
    ...
    - 현재 추정각은 선형(속도 제한) 모델로 명령각을 따라간다.
      ...
      아니라 BNO085 IMU(I2C)로 직접 측정한 실제 각도가 된다(bno085_imu.py).
    ...
    def __init__(
        ...
        # ── IMU(BNO085) 기반 피치 실제각 측정 ──────────────
        # True로 켜면 _update_servo_position()에서 pitch_angle을
        # 선형(속도 제한) 모델 대신 BNO085 실측값으로 갱신한다.
        # (yaw는 현재 90도 고정 축이라 IMU를 사용하지 않고 계속 선형 모델을 쓴다.)
        use_imu: bool = False,
        imu_i2c_addr: int = 0x4A,
        imu_use_rotation_vector: bool = False,
        ...
        self.imu = None
        if self.use_imu:
            from bno085_imu import BNO085IMU

            self.imu = BNO085IMU(
                i2c_addr=imu_i2c_addr,
                use_rotation_vector=imu_use_rotation_vector,
    ...
    def _update_servo_position(self):
        ...
        - pitch_angle : use_imu=True이면 BNO085 실측값을 사용한다.
