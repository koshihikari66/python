...
# 피치 축 두 번째 서보(토크 보강용). 하드웨어 PWM 채널은 라즈베리파이 전체에
# 2개뿐이라 서보가 3개(yaw 1 + pitch 2) 이상이면 하드웨어 PWM으로는 다 못
# 돌린다. 그래서 이 축은 하드웨어 PWM을 포기하고 계속 pigpio(DMA 기반 소프트
# 웨어 PWM)로 구동한다 — pigpio는 여러 GPIO를 같은 타임베이스로 동시에
# 다루므로, 서보가 1개든 2개든 신호 자체의 타이밍/지터는 문제되지 않는다.
# TODO: 아래 GPIO 번호를 실제로 배선한 핀으로 바꾸세요 (BCM 번호 기준).
PITCH_PIN2 = 24
...
class ServoController:
    ...
    피치 축 2서보 구동(pitch_pin2):
    - pitch_pin2를 지정하면 매 프레임 계산되는 피치 명령각을 두 서보에
      동시에(같은 PID 출력으로) 내보낸다. 하드웨어 PWM 채널이 2개뿐이라
      3번째 서보를 하드웨어 PWM으로 못 돌리는 문제를 pigpio 소프트웨어
      PWM으로 우회한 것 — pigpio는 여러 GPIO를 같은 DMA 타임베이스로
      동시에 처리하므로 서보 1개일 때와 비교해 신호 타이밍/지터가
      나빠지지 않는다.
    - 두 서보는 별도 상태(추정각)를 갖지 않고 항상 같은 명령각을 따라가는
      것으로 취급한다. 두 서보의 실제 반응속도가 크게 다르면(다른 모델/
      부하) 기계적으로 서로 힘겨루기를 할 수 있으니, 가능하면 동일 모델을
      쓰고 pitch2_pw_min/mid/max로 중립점 개체차만 보정하는 것을 권장한다.
    - 두 서보를 동시에 움직일 때 순간 전류가 커지므로, 전원(5V 레일)이
      두 서보를 동시에 감당할 만큼 충분한지 확인할 것 — 전원이 처지면
      신호 자체는 멀쩡해도 서보가 버벅이거나 라즈베리파이가 재부팅되는
      것처럼 보일 수 있다.
    """
    ...
    def __init__(
        ...
        # ── 피치 축 두 번째 서보 (한 축 2서보, pigpio 소프트웨어 PWM) ──
        # None이면 기존과 동일하게 서보 1개만 사용한다(하위 호환).
        pitch_pin2: int = None,
        # 두 번째 서보가 첫 번째와 마주보게(반대 방향으로) 장착된 경우 True.
        # 조립 후 실제로 움직여보고 두 서보가 반대로 도는 것 같으면 뒤집는다.
        pitch2_mirrored: bool = False,
        # 두 서보의 중립점(PW_MID) 개체차 보정. 같은 모델이어도 실측 중립
        # 펄스폭이 수십 us씩 다른 경우가 흔하므로, 캘리브레이션 후 값이
        # 다르면 여기에 두 번째 서보의 실측치를 넣는다. None이면 PW_MIN/MID/MAX와 동일.
        pitch2_pw_min: int = None,
        pitch2_pw_mid: int = None,
        pitch2_pw_max: int = None,
        ...
        self.pitch_pin2      = pitch_pin2
        self.pitch2_mirrored = pitch2_mirrored
        self.pitch2_pw_min   = pitch2_pw_min if pitch2_pw_min is not None else PW_MIN
        self.pitch2_pw_mid   = pitch2_pw_mid if pitch2_pw_mid is not None else PW_MID
        self.pitch2_pw_max   = pitch2_pw_max if pitch2_pw_max is not None else PW_MAX
    ...
    def _write_pitch_cmd(self, angle_deg: float):
        ...
        if self.pitch_pin2 is not None:
            angle2 = self.pitch_cmd_angle
            if self.pitch2_mirrored:
                # 반대 방향 장착 보정: 이 축의 범위(PITCH_MIN~PITCH_MAX) 안에서
                # 각도를 반사시켜, 같은 물리적 위치를 만들도록 한다.
                angle2 = (PITCH_MIN + PITCH_MAX) - angle2
            self._set_pw(
                self.pitch_pin2,
                self._angle_to_pw(
                    angle2,
                    pw_min=self.pitch2_pw_min,
                    pw_mid=self.pitch2_pw_mid,
                    pw_max=self.pitch2_pw_max,
                ),
            )
    ...
    def stop(self):
        ...
        if self.pitch_pin2 is not None:
            self._set_pw(self.pitch_pin2, 0)
