...
class ServoController:
    ...
    - 현재 추정각은 1차 지연 모델로 명령각을 따라간다.

        theta_actual[k+1] = theta_actual[k] + a * (theta_cmd[k] - theta_actual[k])
        a = 1 - exp(-dt / tau)
    ...
    - PID 게인(kp/ki/kd), output_limit, integral_limit, deadband,
      cmd_smooth(EMA 스무딩), servo_tau(1차 지연 시정수)까지 전부
    ...
    def __init__(
        ...
        # 1차 지연 모델 시정수 [s], 축별 분리
        # 값이 작을수록 실제각이 명령각을 빨리 따라감.
        # 진동/지연 보정 테스트용 초기값: 0.10~0.20 권장
        yaw_servo_tau: float   = 0.15,
        pitch_servo_tau: float = 0.15,
        ...
        # 1차 지연 모델 시정수도 축별로 분리
        self.yaw_servo_tau   = max(1e-6, yaw_servo_tau)
        self.pitch_servo_tau = max(1e-6, pitch_servo_tau)
    ...
    def _lag_alpha_yaw(self) -> float:
        """
        YAW축 1차 지연 모델 이산화 계수.
        a = 1 - exp(-dt/tau)
        """
        return 1.0 - math.exp(-self.dt / self.yaw_servo_tau)

    def _lag_alpha_pitch(self) -> float:
        """
        PITCH축 1차 지연 모델 이산화 계수.
        a = 1 - exp(-dt/tau)
        """
        return 1.0 - math.exp(-self.dt / self.pitch_servo_tau)

    def _update_lag_model(self):
        """
        명령각을 기준으로 현재 추정각을 1차 지연 모델로 갱신한다.
        이 함수가 갱신하는 yaw_angle / pitch_angle이 '현재 서보모터 각도 추정값'이다.
        축마다 시정수(tau)가 다를 수 있으므로 alpha도 축별로 따로 계산한다.
        """
        a_yaw   = self._lag_alpha_yaw()
        a_pitch = self._lag_alpha_pitch()

        self.yaw_angle   += a_yaw   * (self.yaw_cmd_angle   - self.yaw_angle)
        self.pitch_angle += a_pitch * (self.pitch_cmd_angle - self.pitch_angle)
    ...
    # yaw_angle/pitch_angle은 _update_lag_model()에서 천천히 따라간다.
    ...
    def move(
        ...
        # PWM 명령 후, 현재 서보 각도 추정값을 1차 지연으로 갱신
        self._update_lag_model()
