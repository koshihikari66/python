...
from sccpid_tau import ServoController

# ── 서보 응답 모델 (1차 지연 + 슬루율 제한) ────────────────
SERVO_MAX_SPEED = 90.0   # [deg/s] 실제 짐벌(부하 포함) 실측: 1초에 약 90도
SERVO_TAU = 0.30         # [s] 임시값! 스텝 응답으로 측정해서 교체할 것
CTRL_DT = 1 / 30

# 모델각이 명령각을 뒤따르면 프레임당 실제 이동량이 kp*err의 alpha배로 줄어든다.
# 기존 튜닝(kp/kd = 0.13/0.001, output_limit = 3°/프레임 = 90°/s)과 같은 공칭
# 속도를 유지하도록 SERVO_TAU에서 자동 보정한다. 튜닝은 _KP_BASE/_KD_BASE로 한다.
# output_limit = 명령이 모델각을 앞서는 최대량. 이 값일 때 프레임당 모델 이동량이
# 정확히 max_speed*dt(=슬루율 상한)가 된다.
_SERVO_ALPHA = 1.0 - math.exp(-CTRL_DT / SERVO_TAU)
_KP_BASE = 0.13
_KD_BASE = 0.001

# ── YAW / PITCH 축별 독립 서보 파라미터 ───────────────────
# 두 축의 부하/기어비/마찰이 달라 같은 게인으로는 한쪽이 항상
# 과/부족 튜닝이 되는 문제가 있어 yaw/pitch를 완전히 분리했다.
# I 게인과 EMA 스무딩은 기존 설정값이 0이라 실제 영향이 없어 제거했다.
YAW_PID_KP = _KP_BASE / _SERVO_ALPHA
YAW_PID_KD = _KD_BASE / _SERVO_ALPHA
YAW_PID_OUTPUT_LIMIT = 3   # [deg]
YAW_PID_DEADBAND = 1.0

#pid_output_limit=SERVO_MAX_SPEED * CTRL_DT / _SERVO_ALPHA

PITCH_PID_KP = _KP_BASE / _SERVO_ALPHA
PITCH_PID_KD = _KD_BASE / _SERVO_ALPHA
PITCH_PID_OUTPUT_LIMIT = 3
PITCH_PID_DEADBAND = 1.0
...
servo = ServoController(
    yaw_kp=YAW_PID_KP,
    yaw_kd=YAW_PID_KD,
    yaw_output_limit=YAW_PID_OUTPUT_LIMIT,
    yaw_deadband=YAW_PID_DEADBAND,
    pitch_kp=PITCH_PID_KP,
    pitch_kd=PITCH_PID_KD,
    pitch_output_limit=PITCH_PID_OUTPUT_LIMIT,
    pitch_deadband=PITCH_PID_DEADBAND,
